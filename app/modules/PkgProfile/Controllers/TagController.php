...
use Modules\PkgProfile\App\Requests\TagRequest;
use Modules\PkgProfile\Services\TagService;
use Modules\PkgProfile\Services\ArticleService;
use Illuminate\Http\Request;
use Maatwebsite\Excel\Facades\Excel;
use Modules\PkgProfile\App\Exports\TagExport;
use Modules\PkgProfile\App\Imports\TagImport;
...
class TagController extends AdminController
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            return response()->json([
                'html' => view('PkgProfile::tag.table', compact('data'))->render()
            ]);
        }
    
        // Vue principale pour le chargement initial
        return view('PkgProfile::tag.index', compact('data'));
    }
    ...
    public function create()
    {
        ...
        return view('PkgProfile::tag.create', compact('item', 'articles'));
    }
    ...
    public function store(TagRequest $request)
    {
        ...
        return redirect()->route('tags.index')->with('success', __('Core::msg.addSuccess', [
            'entityToString' => $tag,
            'modelName' => __('PkgProfile::tag.singular')
        ]));
    }
    public function show(string $id)
    {
        $item = $this->tagService->find($id);
        return view('PkgProfile::tag.show', compact('item'));
    }
    ...
    public function edit(string $id)
    {
        $item = $this->tagService->find($id);
        $articles = $this->articleService->all();
        return view('PkgProfile::tag.edit', compact('item', 'articles'));
    }
    ...
    public function import(Request $request)
    {
        ...
        return redirect()->route('tags.index')->with(
            ...
            'modelNames' =>  __('PkgProfile::tag.plural')
            ]));
